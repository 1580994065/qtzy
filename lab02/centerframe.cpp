...
void CenterFrame::createUserCommandArea()
{
    ...
    QImage image(":/new/新建文件夹/icon1.jpg");     //QImage 读取资源图片
    QRect targetRect(0,0,p.size().width(),p.size().height());
    QRect sourceRect =image.rect();
    paint.drawImage(targetRect,image,sourceRect);

    btnImg= new QPushButton(group);     //btnGraph加入组中
    btnImg->setIcon(QIcon(pixmap));
    btnImg->setCheckable(true);             //设置 check 属性
